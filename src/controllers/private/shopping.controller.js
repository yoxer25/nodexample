...
export const getShopping = async (req, res) => {
  ...
  if (user.user !== null) {
    try {
      ...
      res.render("private/shopping/index", { user, shopping });
    } catch (error) {
      const msg = error.message;
      res.render("private/shopping/index", { user });
    }
  } else {
    res.redirect("/");
  }
...
export const getCreateShopping = async (req, res) => {
  ...
  if (user.user !== null) {
    try {
      ...
      res.render("private/shopping/create", { user, products });
    } catch (error) {
      const msg = error.message;
      res.render("private/shopping/create");
    }
  } else {
    res.redirect("/");
  }
...
export const createShopping = async (req, res) => {
  ...
  if (user.user !== null) {
    try {
      ...
      res.redirect("/private/compra");
    } catch (error) {
      const msg = error.message;
      console.log(msg);
      res.redirect("/private/compra/create");
    }
  } else {
    res.redirect("/");
  }
